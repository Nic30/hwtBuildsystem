...
def _parse_configs(defInstance: HwModule, args: List[str], default_params: List[Dict[str, object]]):
    configs_cnt = None
    for p in defInstance._hwParams:
    ...
    for conf in default_params:
        assert len(conf) == len(defInstance._hwParams), (conf, defInstance._hwParams)
        for p in defInstance._hwParams:
            v = conf[p._name]
...
def ban_names_in_serializer(serializer_cls: Type[DummySerializerConfig], reserved_ids: Optional[List[str]]) -> Type:
...
def hwModule_from_cli_args(hwModuleCls: Type[HwModule],
    ...
    """
    :param hwModuleCls: unit class or anything callable which produces instance of HwModule
    ...
    if args.generics:
        stdout.write(str(' '.join([p._name for p in defInstance._hwParams])))
        return
    ...
    multiConfHwModule = MultiConfigHwModuleWrapper(unitVariants)
    to_rtl(multiConfHwModule, store_manager=store_man)
