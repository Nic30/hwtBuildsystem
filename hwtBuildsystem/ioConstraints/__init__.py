...
from hwtBuildsystem.vivado.xdcGen import PackagePin, IoStandard
...
class set_IoPin(iHdlConstrain):
    ...
    def _copy_with_root_upadate(self, old_path_prefix, new_path_prefix):
        raise ValueError("The pin map is appliable only to a top thus no root reallocation should not be valid")
    ...
    def to_xdc(ser: XdcSerializer, self):
        ...
        if p.bits:
            ...
            for b, bm in zip(p.bits, m):
                w(PackagePin(b, bm).asTcl())
                w("\n")
        else:
            assert len(m) == 1, (s, m)
            w(PackagePin(p, m[0]).asTcl())
            w("\n")
...
class set_IoStandard(iHdlConstrain):
    ...
    def to_xdc(ser: XdcSerializer, self):
        ...
        if p.bits:
            for b in p.bits:
                w(IoStandard(b, ioStd).asTcl())
                w("\n")
        else:
            w(IoStandard(p, ioStd).asTcl())
            w("\n")
